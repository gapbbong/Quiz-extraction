...
YEAR = "2019"
SESSION = "2" # Updated for Session 2
...
image_dir = r'd:\App\Quiz-extraction\session_images\s2' # Path for S2
output_dir = r'd:\App\Quiz-extraction\output\s2'
...
for page_file in page_files:
    ...
    try:
        response = model.generate_content([prompt_template.format(year=YEAR, session=SESSION, exam_title="2019년 제2회 CBT 기출복원문제")] + [p_img])
...
# Save final JSON for Session 2
with open(os.path.join(output_dir, 'ElectricExam2019_S2.json'), 'w', encoding='utf-8') as f:
    json.dump(final_data, f, ensure_ascii=False, indent=2)

print(f"Session 2 extraction complete. Total questions: {len(final_data)}")
